...
void PanelGui::displayStatus(){
    ...
    if(panel->isSSOnline()){
        ...
        mvwprintw(mainWindow, 0, 48, (" svdss[" + QString::number(getuid()) + "]:  ONLINE (" + QHostInfo::localHostName() + ")").toUtf8());
        wattroff(mainWindow, C_SS_STATUS_ON);
    } else {
        wattron(mainWindow, C_SS_STATUS_OFF);
        mvwprintw(mainWindow, 0, 48, (" svdss[" + QString::number(getuid()) + "]: OFFLINE (" + QHostInfo::localHostName() + ")").toUtf8());
